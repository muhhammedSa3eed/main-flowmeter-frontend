...
//   { id: 2, title: "3000 ms", value: 3000 },
//   { id: 3, title: "5000 ms", value: 5000 },
//   { id: 4, title: "7000 ms", value: 7000 },
// ];
...
export default function EditConnections({
  ...
  const form = useForm<z.infer<typeof ConnectionSchema>>({
    ...
    defaultValues: {
      ...
      property: {
        ...
        databaseName: parsedProperty.databaseName,
        method: parsedProperty.method,
        format: parsedProperty.format,
        address: parsedProperty.address,
        ip: parsedProperty.ip,
        port: parsedProperty.port,
        host: parsedProperty.host,
        user: parsedProperty.user,
  ...
  async function onSubmit(values: z.infer<typeof ConnectionSchema>) {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/devices/${device.id}`,
        {
          method: "PUT",
          headers: {
  ...
  async function testConnection(values: z.infer<typeof ConnectionSchema>) {
    console.log("Running testConnection with update:", values);
    try {
      const response = await fetch(
